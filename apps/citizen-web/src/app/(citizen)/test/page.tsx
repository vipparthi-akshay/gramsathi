...
export default function TestPage() {
  ...
  const runSingleTest = async (index: number) => {
    ...
    setResults(prev => prev.map((r, i) => i === index ? { ...r, status: 'pending', message: 'Running...' } : r));
    ...
    try {
      ...
      switch (index) {
        case 0: {
          ...
          msg = success ? `OK (${res.status})` : `Unexpected status ${res.status}`;
          break;
        }
        case 1: {
          const res = await axios.get(`${apiUrl}/api/auth/health`, { timeout: 5000 });
          success = res.status === 200;
          msg = success ? `Auth service reachable` : `Status ${res.status}`;
          break;
        }
        case 2: {
          const res = await axios.get(`${apiUrl}/api/schemes/health`, { timeout: 5000 });
          success = res.status === 200;
          msg = success ? `Scheme service reachable` : `Status ${res.status}`;
          break;
        }
        case 3: {
          const res = await axios.get(`${apiUrl}/api/documents/health`, { timeout: 5000 });
          success = res.status === 200;
          msg = success ? `Document service reachable` : `Status ${res.status}`;
          break;
        }
        case 4: {
          const res = await axios.get(`${apiUrl}/api/ai/health`, { timeout: 5000 });
          success = res.status === 200;
          msg = success ? `AI chat service reachable` : `Status ${res.status}`;
          break;
        }
        case 5: {
          const res = await axios.get(`${apiUrl}/api/notifications/health`, { timeout: 5000 });
          success = res.status === 200;
          msg = success ? `Notification service reachable` : `Status ${res.status}`;
          break;
        }
        case 6: {
          const res = await axios.get(`${apiUrl}/api/db/health`, { timeout: 5000 });
          success = res.status === 200;
          msg = success ? `Database connected` : `Status ${res.status}`;
          break;
        }
        case 7: {
          const res = await axios.get(`${apiUrl}/api/cache/health`, { timeout: 5000 });
          success = res.status === 200;
      ...
      setResults(prev => prev.map((r, i) =>
        i === index ? { ...r, status: success ? 'pass' : 'fail', message: msg, duration } : r
      ));
      if (success) {
      ...
      const duration = Math.round(performance.now() - start);
      setResults(prev => prev.map((r, i) =>
        i === index ? { ...r, status: 'fail', message: err.message || 'Request failed', duration } : r
      ));
  ...
  const resetTests = () => {
    setResults(prev => prev.map(r => ({ ...r, status: 'pending' as const, message: 'Not tested', duration: undefined })));
  };

  const getStatusIcon = (status: string) => {
    switch (status) {
      case 'pass': return <CheckCircleIcon sx={{ color: 'success.main' }} />;
      case 'fail': return <CancelIcon sx={{ color: 'error.main' }} />;
      default: return <Box sx={{ width: 24, height: 24 }} />;
    }
  };
    ...
    <Box sx={{ minHeight: '100vh', pb: 12, backgroundColor: 'background.default' }}>
      <Container maxWidth="lg" sx={{ px: 2, pt: 3 }}>
        ...
        <Alert severity="warning" sx={{ borderRadius: 3, mb: 3 }}>
          ...
          This dashboard runs live tests against the backend services. Ensure the API server is running before executing tests.
        </Alert>
        ...
        <Card sx={{ borderRadius: 3, mb: 3 }}>
          <CardContent sx={{ p: 3 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
              ...
              <Typography variant="subtitle1" fontWeight={600}>API Endpoint</Typography>
            </Box>
            <TextField
              fullWidth
              size="small"
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          <Grid item xs={4}>
            <Card sx={{ borderRadius: 3, bgcolor: 'success.50' }}>
              <CardContent sx={{ p: 2, textAlign: 'center' }}>
                <Typography variant="h4" fontWeight={700} color="success.main">{passCount}</Typography>
          ...
          <Grid item xs={4}>
            <Card sx={{ borderRadius: 3, bgcolor: 'error.50' }}>
              <CardContent sx={{ p: 2, textAlign: 'center' }}>
                <Typography variant="h4" fontWeight={700} color="error.main">{failCount}</Typography>
          ...
          <Grid item xs={4}>
            <Card sx={{ borderRadius: 3, bgcolor: 'grey.50' }}>
              <CardContent sx={{ p: 2, textAlign: 'center' }}>
                <Typography variant="h4" fontWeight={700} color="text.secondary">{pendingCount}</Typography>
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell><Typography fontWeight={600}>Test</Typography></TableCell>
                ...
                <TableCell align="right"><Typography fontWeight={600}>Duration</Typography></TableCell>
            ...
            <TableBody>
              {results.map((result, idx) => (
                <TableRow key={result.name} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                  ...
                  <TableCell>
                    <Typography variant="body2" fontWeight={500}>{result.name}</Typography>
                  </TableCell>
                  <TableCell>
                    <Chip
                      label={result.status === 'pass' ? 'PASS' : result.status === 'fail' ? 'FAIL' : 'PENDING'}
                      size="small"
                  ...
                  <TableCell align="right">
                    <Tooltip title={`Run ${result.name}`}>
                      <IconButton size="small" onClick={() => runSingleTest(idx)} disabled={running}>
        ...
        <Card sx={{ borderRadius: 3, mb: 3 }}>
          <CardContent sx={{ p: 3 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 2 }}>
              ...
              <Typography variant="subtitle1" fontWeight={600}>Environment Info</Typography>
            </Box>
            <Grid container spacing={2}>
              <Grid item xs={12} sm={6}>
                <Typography variant="caption" color="text.secondary">Next.js Version</Typography>
                <Typography variant="body2">14.1.0</Typography>
              </Grid>
              <Grid item xs={12} sm={6}>
                <Typography variant="caption" color="text.secondary">MUI Version</Typography>
                <Typography variant="body2">5.15.6</Typography>
              </Grid>
              <Grid item xs={12} sm={6}>
                <Typography variant="caption" color="text.secondary">API Base URL</Typography>
                <Typography variant="body2">{apiUrl}</Typography>
              </Grid>
              <Grid item xs={12} sm={6}>
                <Typography variant="caption" color="text.secondary">Build Time</Typography>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2, flexWrap: 'wrap', mb: 4 }}>
          <Button variant="outlined" onClick={() => setShowDetails(!showDetails)} startIcon={showDetails ? <VisibilityOffIcon /> : <VisibilityIcon />}>
        ...
        {showDetails && (
          <Card sx={{ borderRadius: 3, mb: 3, bgcolor: 'grey.900' }}>
            <CardContent sx={{ p: 3 }}>
              <Typography variant="subtitle2" color="grey.400" sx={{ mb: 1, fontFamily: 'monospace' }}>
                Debug Output
              </Typography>
              <Box
                component="pre"
