...
export default function LoginPage() {
  ...
  const handleSendOtp = () => {
    if (mobile.length !== 10 || isNaN(Number(mobile))) {
      setError(t('login.invalidMobile', 'Please enter a valid 10-digit number'));
      return;
    }
    setError('');
    setStep('otp');
  ...
  return (
    ...
    >
      ...
      <Box sx={{ position: 'absolute', top: 16, right: 16, zIndex: 10 }}>
        ...
        <GramLanguageSelector open={langOpen} onClose={() => setLangOpen(false)} />
      </Box>

      <Container maxWidth="xs" sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', zIndex: 1, py: 4 }}>
        <motion.div
              ...
              {step === 'mobile' ? (
                ...
                >
                  <Typography variant="h6" fontWeight={600} sx={{ mb: 2, display: 'flex', alignItems: 'center', gap: 1 }}>
                    <PhoneIphoneIcon color="primary" /> {t('login.enterMobile', 'Enter Mobile Number')}
                  </Typography>

                  <TextField
                    fullWidth
                    autoFocus
                    placeholder={t('login.mobilePlaceholder', '10-digit mobile number')}
                    variant="outlined"
                    type="tel"
                    value={mobile}
                    onChange={(e) => {
                    ...
                    InputProps={{
                      ...
                      sx: { borderRadius: 2, fontSize: '1.1rem', letterSpacing: 1 }
                    }}
                    sx={{ mb: 3 }}
                  />
                ...
                >
                  <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                    <IconButton size="small" onClick={() => { setStep('mobile'); setError(''); }} sx={{ mr: 1, ml: -1 }}>
                      <ArrowBackIcon />
                    </IconButton>
                    <Typography variant="h6" fontWeight={600} sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                      <VerifiedUserIcon color="primary" /> {t('login.enterOtp', 'Enter OTP')}
                    </Typography>
                  </Box>

                  <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
                    {t('login.otpSent', 'OTP sent to +91')} {mobile}.{' '}
                    <Box component="span" sx={{ color: 'primary.main', cursor: 'pointer', fontWeight: 500 }} onClick={() => setStep('mobile')}>
