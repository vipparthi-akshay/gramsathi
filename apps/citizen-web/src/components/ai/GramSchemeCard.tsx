...
const MatchBadge = styled(Box)(({ theme }) => ({
  ...
  padding: '4px 10px',
  borderRadius: 28,
  backgroundColor: (theme.palette as any).primaryContainer || 'rgba(21, 101, 192, 0.1)',
  width: 'fit-content',
}));
...
export default function GramSchemeCard({
  ...
  const scoreColor =
    matchScore >= 80 ? 'success.main' : matchScore >= 50 ? 'warning.main' : 'error.main';
    ...
    <GramCard
      ...
      actions={
        <Box sx={{ display: 'flex', gap: 1, width: '100%', justifyContent: 'space-between' }}>
          ...
          {onShare && (
            <IconButton size="small" onClick={onShare} aria-label="Share scheme">
