...
const mockCitizen = {
  ...
  village: "Purwa",
  address:
    "Village Purwa, Block Sadar, District Lucknow, Uttar Pradesh - 226001",
  isVerified: true,
  ...
  applications: [
    {
      id: "APP-001",
      schemeName: "Kisan Samman Yojana",
      status: "approved",
  ...
  return (
    <Box>
      <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 3 }}>
        <IconButton onClick={() => navigate("/citizens")}>
          <ArrowBackIcon />
        </IconButton>
        <Box sx={{ flex: 1 }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
            ...
            <StatusBadge
              status={mockCitizen.isVerified ? "verified" : "unverified"}
              size="medium"
            />
          </Box>
              ...
              <List dense disablePadding>
                {mockCitizen.documents.map((doc, idx) => (
                  <ListItem key={idx} sx={{ px: 0 }}>
                    ...
                    <ListItemText
                      primary={doc.name}
                      secondary={doc.verified ? "Verified" : "Pending"}
                    />
                  </ListItem>
                ))}
              </List>
        ...
        <Grid item xs={12} md={8}>
          ...
          <Card>
            <CardContent>
              ...
              >
                <Tab
                  label={`Applications (${mockCitizen.applications.length})`}
                />
                ...
                <List dense disablePadding>
                  {mockCitizen.grievances.map((grv) => (
                    <ListItem
                      key={grv.id}
                      sx={{ px: 0, cursor: "pointer" }}
                      onClick={() => navigate(`/grievances/${grv.id}`)}
                    >
                      <ListItemText
