...
export default function SchemeForm() {
  ...
  const [criteriaEntries, setCriteriaEntries] = useState<
  ...
  const addCriteriaEntry = () =>
    setCriteriaEntries([...criteriaEntries, { key: "", value: "" }]);
  const updateCriteriaEntry = (
    idx: number,
    field: "key" | "value",
    value: string,
  ) => {
  ...
  const removeCriteriaEntry = (idx: number) => {
    if (criteriaEntries.length > 1)
      setCriteriaEntries(criteriaEntries.filter((_, i) => i !== idx));
  };

  return (
    <Box>
      <Box sx={{ display: "flex", alignItems: "center", gap: 1, mb: 3 }}>
        <IconButton onClick={() => navigate("/schemes")}>
          <ArrowBackIcon />
        </IconButton>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            {isEdit ? "Update scheme details" : "Add a new government scheme"}
          </Typography>
        </Box>
      </Box>
                ...
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} md={6}>
                    <TextField
                      ...
                      label="Name (Marathi)"
                      value={form.nameMr}
                      onChange={(e) => updateField("nameMr", e.target.value)}
                    />
                  </Grid>
                  <Grid item xs={12} md={6}>
                    <TextField
                      ...
                      label="Name (Tamil)"
                      value={form.nameTa}
                      onChange={(e) => updateField("nameTa", e.target.value)}
                    />
                  </Grid>
                  <Grid item xs={12}>
                    <TextField
                ...
                {criteriaEntries.map((entry, idx) => (
                  ...
                  >
                    <TextField
                      ...
                      value={entry.key}
                      onChange={(e) =>
                        updateCriteriaEntry(idx, "key", e.target.value)
                      }
                      sx={{ flex: 1 }}
                    ...
                    <TextField
                      ...
                      value={entry.value}
                      onChange={(e) =>
                        updateCriteriaEntry(idx, "value", e.target.value)
                      }
                      sx={{ flex: 1 }}
                      placeholder="e.g., <3L"
                    />
                    <IconButton
                      size="small"
                      onClick={() => removeCriteriaEntry(idx)}
                      color="error"
                    >
            ...
            <Card>
              <CardContent>
                ...
                <Autocomplete
                  ...
                  onChange={(_, val) =>
                    updateField("documentRequirements", val)
                  }
          ...
          <Grid item xs={12} lg={4}>
            ...
            <Card sx={{ mb: 2.5 }}>
              <CardContent>
                ...
                {!form.name && (
                  <Typography variant="body2" color="text.secondary">
                    Fill in scheme details to see preview
                  </Typography>
                )}
              </CardContent>
            </Card>
