...
export default function Citizens() {
  ...
  useEffect(() => {
    ...
    if (filters.status)
      filtered = filtered.filter((c) =>
        filters.status === "verified" ? c.isVerified : !c.isVerified,
      );
      ...
      >
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            View and manage citizen profiles
          </Typography>
        </Box>
      </Box>
        ...
        <GramTable
          columns={[
            ...
            },
            {
              id: "phone",
              label: "Phone",
              render: (row: Citizen) => row.phone,
            },
            {
              id: "aadhaar",
              label: "Aadhaar",
              render: (row: Citizen) => row.aadhaar,
            },
            {
              id: "location",
              label: "Location",
            ...
            },
            {
              id: "language",
              label: "Language",
              render: (row: Citizen) => row.preferredLanguage,
            },
            {
              id: "age",
              label: "Age",
              render: (row: Citizen) => row.age,
              sortable: true,
            },
            {
              id: "verified",
          ...
          actions={(row: Citizen) => (
            <Box sx={{ display: "flex", gap: 0.5 }}>
              <Tooltip title="View Profile">
                <IconButton
                  size="small"
                  onClick={() => navigate(`/citizens/${row.id}`)}
                >
